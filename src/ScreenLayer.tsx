...
import type { NavigateOptions, PreloadHandle, ScreenEntry, ScreenNavigator } from './types'
import type { ScreenStackActions } from './utils/useScreenStack'

interface ScreenLayerProps {
  entry: ScreenEntry
  level: number
  baseClass: string
  actions: ScreenStackActions
...
export const ScreenLayer: React.FC<ScreenLayerProps> = ({
  ...
  // Each ScreenLayer exposes a navigator bound to its own id so that
  // nextScreen/backScreen automatically know who the parent is.
  const navigator: ScreenNavigator = useMemo(
    () => ({
      nextScreen: (options: NavigateOptions | PreloadHandle) =>
        actions.push(options, entry.id),
      backScreen: () => actions.pop(entry.id),
      preloadScreen: (options: NavigateOptions) => actions.preload(options, entry.id),
      deleteScreens: (pageNames: string[]) => actions.deleteByNames(pageNames),
    }),
